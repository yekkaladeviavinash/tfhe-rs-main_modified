                ...
                while t > 16 {
                    ...
                    for (data, (&w, &w_shoup)) in zip(data.chunks_exact_mut(2 * t), zip(w, w_shoup)) {
                        ...
                        if t >= 16 {
                            ...
                            let w_shoup = simd.splat_u32x16(w_shoup);
                            for (z0_, z1_) in zip(z0, z1) {
                                let z0_vec = simd.splat_u32x16(*z0_);
                                let z1_vec = simd.splat_u32x16(*z1_);
                                let (r0, r1) = butterfly(simd, z0_vec, z1_vec, w, w_shoup, p, neg_p, two_p);
                                let r0: [u32; 16] = cast(r0);
                                let r1: [u32; 16] = cast(r1);
                                *z0_ = r0[0];
                                *z1_ = r1[0];
                            }
                        }
        ...
        for Impl<'_, B, LB>
    {
        ...
        fn call(self) -> Self::Output {
            ...
            if n <= RECURSION_THRESHOLD {
                ...
                let w1 = &twid[w_idx1..];
                let w1_shoup = &twid_shoup[w_idx1..];

                {
                    ...
                    for (data, (&w, &w_shoup)) in zip(data.chunks_exact_mut(2 * t1), zip(w1, w1_shoup)) {
...

                {
                    ...
                    let w = simd.splat_u32x16(twid[w_idx2_base]);
                    let w_shoup = simd.splat_u32x16(twid_shoup[w_idx2_base]);
                    ...
                    let w = simd.splat_u32x16(twid[w_idx2_base + 1]);
                    let w_shoup = simd.splat_u32x16(twid_shoup[w_idx2_base + 1]);
                ...
                while t > 8 {
                    ...
                    for (data, (&w, &w_shoup)) in zip(data.chunks_exact_mut(2 * t), zip(w, w_shoup)) {
                        ...
                        if t >= 8 {
                            ...
                            let w_shoup = simd.splat_u32x8(w_shoup);
                            for (z0_, z1_) in zip(z0, z1) {
                                let z0_vec = simd.splat_u32x8(*z0_);
                                let z1_vec = simd.splat_u32x8(*z1_);
                                let (r0, r1) = butterfly(simd, z0_vec, z1_vec, w, w_shoup, p, neg_p, two_p);
                                let r0: [u32; 8] = cast(r0);
                                let r1: [u32; 8] = cast(r1);
                                *z0_ = r0[0];
                                *z1_ = r1[0];
                            }
                        }
        ...
        fn call(self) -> Self::Output {
            ...
            if n <= RECURSION_THRESHOLD {
...

                {
                    ...
                    let w = simd.splat_u32x8(twid[w_idx2_base]);
                    let w_shoup = simd.splat_u32x8(twid_shoup[w_idx2_base]);
                    let (z0, z1) = data_half0.split_at_mut(t2);
                    let z0 = as_arrays_mut::<8, _>(z0).0;
                    let z1 = as_arrays_mut::<8, _>(z1).0;
                    for (z0_, z1_) in zip(z0, z1) {
                    ...
                    let w = simd.splat_u32x8(twid[w_idx2_base + 1]);
                    let w_shoup = simd.splat_u32x8(twid_shoup[w_idx2_base + 1]);
                    let (z0, z1) = data_half1.split_at_mut(t2);
                    let z0 = as_arrays_mut::<8, _>(z0).0;
                    let z1 = as_arrays_mut::<8, _>(z1).0;
                    for (z0_, z1_) in zip(z0, z1) {
                ...
                for (data, (&w, &w_shoup)) in zip(data.chunks_exact_mut(2 * t), zip(w, w_shoup)) {
                    ...
                    if t >= 16 {
                        ...
                        for (z0_, z1_) in zip(z0, z1) {
                            ...
                            let mut z1 = cast(*z1_);
                            (z0, z1) = if use_last {
                                last_butterfly(simd, z0, z1, w, w_shoup, p, neg_p, two_p)
                            } else {
                                butterfly(simd, z0, z1, w, w_shoup, p, neg_p, two_p)
                            };
                            *z0_ = cast(z0);
                            *z1_ = cast(z1);
                        }
                        ...
                        for (z0_, z1_) in zip(z0, z1) {
                            let z0_vec = simd.splat_u32x16(*z0_);
                            let z1_vec = simd.splat_u32x16(*z1_);
                            let (r0, r1) = if use_last {
                                last_butterfly(simd, z0_vec, z1_vec, w, w_shoup, p, neg_p, two_p)
                            } else {
                                butterfly(simd, z0_vec, z1_vec, w, w_shoup, p, neg_p, two_p)
                            };
                            let r0: [u32; 16] = cast(r0);
                            let r1: [u32; 16] = cast(r1);
                            *z0_ = r0[0];
        ...
        fn call(self) -> Self::Output {
            ...
            if n <= RECURSION_THRESHOLD {
...

                {
                    ...
                    let w = simd.splat_u32x16(inv_twid[w_idx2_base]);
                    let w_shoup = simd.splat_u32x16(inv_twid_shoup[w_idx2_base]);
                    let (z0, z1) = data_half0.split_at_mut(t2);
                    let z0 = as_arrays_mut::<16, _>(z0).0;
                    let z1 = as_arrays_mut::<16, _>(z1).0;
                    for (z0_, z1_) in zip(z0, z1) {
                    ...
                    let w = simd.splat_u32x16(inv_twid[w_idx2_base + 1]);
                    let w_shoup = simd.splat_u32x16(inv_twid_shoup[w_idx2_base + 1]);
                    let (z0, z1) = data_half1.split_at_mut(t2);
                    let z0 = as_arrays_mut::<16, _>(z0).0;
                    let z1 = as_arrays_mut::<16, _>(z1).0;
                    for (z0_, z1_) in zip(z0, z1) {
                ...
                let w1_shoup = &inv_twid_shoup[w_idx1..];
        ...
        fn call(self) -> Self::Output {
            ...
            if n <= RECURSION_THRESHOLD {
...

                {
                    ...
                    let w = simd.splat_u32x8(inv_twid[w_idx2_base]);
                    let w_shoup = simd.splat_u32x8(inv_twid_shoup[w_idx2_base]);
                    let (z0, z1) = data_half0.split_at_mut(t2);
                    let z0 = as_arrays_mut::<8, _>(z0).0;
                    let z1 = as_arrays_mut::<8, _>(z1).0;
                    for (z0_, z1_) in zip(z0, z1) {
                    ...
                    let w = simd.splat_u32x8(inv_twid[w_idx2_base + 1]);
                    let w_shoup = simd.splat_u32x8(inv_twid_shoup[w_idx2_base + 1]);
                    let (z0, z1) = data_half1.split_at_mut(t2);
                    let z0 = as_arrays_mut::<8, _>(z0).0;
                    let z1 = as_arrays_mut::<8, _>(z1).0;
                    for (z0_, z1_) in zip(z0, z1) {
...
pub(crate) fn fwd_depth_first_scalar_r4(
    ...
    if n <= RECURSION_THRESHOLD {
...

        {
            ...
            let w_shoup = twid_shoup[w_idx2_base];
            let (z0, z1) = data_half0.split_at_mut(t2);
            for (z0_, z1_) in zip(z0, z1) {
                let mut z0 = *z0_;
                let mut z1 = *z1_;
            ...
            let w_shoup = twid_shoup[w_idx2_base + 1];
            let (z0, z1) = data_half1.split_at_mut(t2);
            for (z0_, z1_) in zip(z0, z1) {
                let mut z0 = *z0_;
                let mut z1 = *z1_;
