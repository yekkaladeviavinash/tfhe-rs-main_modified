...
#[allow(unused_imports)]
use self::{
    add_avx2_r4 as add_avx2,
    add_avx512_r4 as add_avx512,
    add_r4 as add,
    ...
    mul_avx2_r4 as mul_avx2,
    mul_avx512_r4 as mul_avx512,
    mul_r4 as mul,
    sub_avx2_r4 as sub_avx2,
    sub_avx512_r4 as sub_avx512,
    sub_r4 as sub,
};

#[inline(always)]
pub(crate) fn add_r4(p: u32, a: u32, b: u32) -> u32 {
    let neg_b = p - b;
    if a >= neg_b {
        a - neg_b
    } else {
...
#[inline(always)]
pub(crate) fn sub_r4(p: u32, a: u32, b: u32) -> u32 {
    let neg_b = p - b;
    if a >= b {
        a - b
    } else {
...
#[inline(always)]
pub(crate) fn mul_r4(p: Div32, a: u32, b: u32) -> u32 {
    Div32::rem_u64(a as u64 * b as u64, p)
}

#[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
#[inline(always)]
fn add_avx2_r4(simd: crate::V3, p: u32x8, a: u32x8, b: u32x8) -> u32x8 {
...
#[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
#[inline(always)]
fn sub_avx2_r4(simd: crate::V3, p: u32x8, a: u32x8, b: u32x8) -> u32x8 {
    let neg_b = simd.wrapping_sub_u32x8(p, b);
...
#[cfg(feature = "avx512")]
#[inline(always)]
fn add_avx512_r4(simd: crate::V4, p: u32x16, a: u32x16, b: u32x16) -> u32x16 {
...
#[cfg(feature = "avx512")]
#[inline(always)]
fn sub_avx512_r4(simd: crate::V4, p: u32x16, a: u32x16, b: u32x16) -> u32x16 {
    let neg_b = simd.wrapping_sub_u32x16(p, b);
...
pub(crate) fn fwd_breadth_first_scalar_r4(
    data: &mut [u32],
    p: u32,
    p_div: Div32,
    twid: &[u32],
...
pub(crate) fn fwd_depth_first_scalar_r4(
    data: &mut [u32],
    p: u32,
    p_div: Div32,
    twid: &[u32],
    ...
    if n <= RECURSION_THRESHOLD {
        fwd_breadth_first_scalar_r4(data, p, p_div, twid, recursion_depth, recursion_half);
    } else {
        let t1 = n / 2;
        let w_idx1 = (1 << recursion_depth) + recursion_half;
        let w1 = &twid[w_idx1..];

        for (data, &w) in zip(data.chunks_exact_mut(2 * t1), w1) {
            let (z0, z1) = data.split_at_mut(t1);
            for (z0, z1) in zip(z0, z1) {
                let z1w = mul(p_div, *z1, w);
                (*z0, *z1) = (add(p, *z0, z1w), sub(p, *z0, z1w));
            }
        }

        let t2 = n / 4;
        let w_idx2_base = (1 << (recursion_depth + 1)) + recursion_half * 2;
        let (data_half0, data_half1) = data.split_at_mut(n / 2);

        {
            let w = twid[w_idx2_base];
            let (z0, z1) = data_half0.split_at_mut(t2);
            for (z0, z1) in zip(z0, z1) {
                let z1w = mul(p_div, *z1, w);
                (*z0, *z1) = (add(p, *z0, z1w), sub(p, *z0, z1w));
            }

            let w = twid[w_idx2_base + 1];
            let (z0, z1) = data_half1.split_at_mut(t2);
            for (z0, z1) in zip(z0, z1) {
                let z1w = mul(p_div, *z1, w);
                (*z0, *z1) = (add(p, *z0, z1w), sub(p, *z0, z1w));
            }
        ...
        let quarter = n / 4;
        let (data0, rest) = data.split_at_mut(quarter);
        let (data1, rest) = rest.split_at_mut(quarter);
        let (data2, data3) = rest.split_at_mut(quarter);

        fwd_depth_first_scalar_r4(
            data0,
            p,
            p_div,
            twid,
...
pub(crate) fn inv_breadth_first_scalar_r4(
    data: &mut [u32],
    p: u32,
    p_div: Div32,
    inv_twid: &[u32],
...
pub(crate) fn inv_depth_first_scalar_r4(
    data: &mut [u32],
    p: u32,
    p_div: Div32,
    inv_twid: &[u32],
    ...
    debug_assert!(n.is_power_of_two());
    if n <= RECURSION_THRESHOLD {
        inv_breadth_first_scalar_r4(data, p, p_div, inv_twid, recursion_depth, recursion_half);
    } else {
        let quarter = n / 4;
        let (data0, rest) = data.split_at_mut(quarter);
        let (data1, rest) = rest.split_at_mut(quarter);
        ...
        {
            let w = inv_twid[w_idx2_base];
            let (z0, z1) = data_half0.split_at_mut(t2);
            for (z0, z1) in zip(z0, z1) {
                (*z0, *z1) = (add(p, *z0, *z1), mul(p_div, sub(p, *z0, *z1), w));
            }

            let w = inv_twid[w_idx2_base + 1];
            let (z0, z1) = data_half1.split_at_mut(t2);
            for (z0, z1) in zip(z0, z1) {
                (*z0, *z1) = (add(p, *z0, *z1), mul(p_div, sub(p, *z0, *z1), w));
            }
...
#[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
pub(crate) fn fwd_breadth_first_avx2_r4(
    simd: crate::V3,
    data: &mut [u32],
    p: u32,
...
#[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
pub(crate) fn fwd_depth_first_avx2_r4(
    simd: crate::V3,
    data: &mut [u32],
    p: u32,
    p_div: (u32, u32, u32, u32),
    ...
    if n <= RECURSION_THRESHOLD {
        fwd_breadth_first_avx2_r4(simd, data, p, p_div, twid, recursion_depth, recursion_half);
    } else {
        let t1 = n / 2;
        let w_idx1 = (1 << recursion_depth) + recursion_half;
        let w1 = &twid[w_idx1..];
        ...
        {
            let p = simd.splat_u32x8(p_scalar);
            let p_div0 = simd.splat_u32x8(p_div.0);
            let p_div1 = simd.splat_u32x8(p_div.1);
            let p_div2 = simd.splat_u32x8(p_div.2);
            let p_div3 = simd.splat_u32x8(p_div.3);
            ...
            for (data, &w) in zip(data.chunks_exact_mut(2 * t1), w1) {
                ...
                if t1 >= 8 {
                    ...
                    for (z0_, z1_) in zip(z0, z1) {
                        let mut z0 = cast(*z0_);
                        let mut z1 = cast(*z1_);
                        let z1w = mul_avx2(simd, p, p_div0, p_div1, p_div2, p_div3, z1, w);
                        (z0, z1) = (add_avx2(simd, p, z0, z1w), sub_avx2(simd, p, z0, z1w));
                        *z0_ = cast(z0);
                        *z1_ = cast(z1);
                    }
                    ...
                    for (z0, z1) in zip(z0, z1) {
                        let z1w = mul(Div32::new(p_scalar), *z1, w);
                        (*z0, *z1) = (add(p_scalar, *z0, z1w), sub(p_scalar, *z0, z1w));
                    }
                }
            }
        }
...
#[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
pub(crate) fn inv_breadth_first_avx2_r4(
    simd: crate::V3,
    data: &mut [u32],
    p: u32,
    p_div: (u32, u32, u32, u32),
...
#[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
pub(crate) fn inv_depth_first_avx2_r4(
    simd: crate::V3,
    data: &mut [u32],
    p: u32,
    p_div: (u32, u32, u32, u32),
    ...
    if n <= RECURSION_THRESHOLD {
        inv_breadth_first_avx2_r4(simd, data, p, p_div, inv_twid, recursion_depth, recursion_half);
    } else {
        let quarter = n / 4;
        let (data0, rest) = data.split_at_mut(quarter);
        let (data1, rest) = rest.split_at_mut(quarter);
...

        {
            ...
            if t2 >= 8 {
                ...
                for (z0, z1) in zip(z0, z1) {
                    (*z0, *z1) = (
                        add(p_scalar, *z0, *z1),
                        mul(Div32::new(p_scalar), sub(p_scalar, *z0, *z1), w),
                    );
                }
            }
...
#[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
#[cfg(feature = "avx512")]
pub(crate) fn fwd_breadth_first_avx512_r4(
    simd: crate::V4,
    data: &mut [u32],
    p: u32,
    p_div: (u32, u32, u32, u32),
...
#[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
#[cfg(feature = "avx512")]
pub(crate) fn fwd_depth_first_avx512_r4(
    simd: crate::V4,
    data: &mut [u32],
    p: u32,
    p_div: (u32, u32, u32, u32),
    ...
    if n <= RECURSION_THRESHOLD {
        fwd_breadth_first_avx512_r4(simd, data, p, p_div, twid, recursion_depth, recursion_half);
    } else {
        let t1 = n / 2;
        let w_idx1 = (1 << recursion_depth) + recursion_half;
        let w1 = &twid[w_idx1..];
        ...
        {
            let p = simd.splat_u32x16(p_scalar);
            let p_div0 = simd.splat_u32x16(p_div.0);
            let p_div1 = simd.splat_u32x16(p_div.1);
            let p_div2 = simd.splat_u32x16(p_div.2);
            let p_div3 = simd.splat_u32x16(p_div.3);
            ...
            for (data, &w) in zip(data.chunks_exact_mut(2 * t1), w1) {
                ...
                if t1 >= 16 {
                    ...
                    for (z0_, z1_) in zip(z0, z1) {
                        let mut z0 = cast(*z0_);
                        let mut z1 = cast(*z1_);
                        let z1w = mul_avx512(simd, p, p_div0, p_div1, p_div2, p_div3, z1, w);
                        (z0, z1) = (add_avx512(simd, p, z0, z1w), sub_avx512(simd, p, z0, z1w));
                        *z0_ = cast(z0);
                        *z1_ = cast(z1);
                    }
...
#[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
#[cfg(feature = "avx512")]
pub(crate) fn inv_breadth_first_avx512_r4(
    simd: crate::V4,
    data: &mut [u32],
    p: u32,
    p_div: (u32, u32, u32, u32),
...
#[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
#[cfg(feature = "avx512")]
pub(crate) fn inv_depth_first_avx512_r4(
    simd: crate::V4,
    data: &mut [u32],
    p: u32,
    p_div: (u32, u32, u32, u32),
    ...
    if n <= RECURSION_THRESHOLD {
        inv_breadth_first_avx512_r4(simd, data, p, p_div, inv_twid, recursion_depth, recursion_half);
    } else {
        let quarter = n / 4;
        let (data0, rest) = data.split_at_mut(quarter);
        let (data1, rest) = rest.split_at_mut(quarter);
...
pub(crate) fn fwd_scalar_r4(data: &mut [u32], p: u32, p_div: Div32, twid: &[u32]) {
    fwd_depth_first_scalar_r4(data, p, p_div, twid, 0, 0);
}
pub(crate) fn inv_scalar_r4(data: &mut [u32], p: u32, p_div: Div32, inv_twid: &[u32]) {
    inv_depth_first_scalar_r4(data, p, p_div, inv_twid, 0, 0);
}

#[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
pub(crate) fn fwd_avx2_r4(simd: crate::V3, data: &mut [u32], p: u32, p_div: Div32, twid: &[u32]) {
    ...
    fwd_depth_first_avx2_r4(simd, data, p, p_div, twid, 0, 0);
}
#[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
pub(crate) fn inv_avx2_r4(simd: crate::V3, data: &mut [u32], p: u32, p_div: Div32, inv_twid: &[u32]) {
    let p_div = p_div.double_reciprocal;
    let p_div = (
        p_div as u32,
        (p_div >> 32) as u32,
    ...
    inv_depth_first_avx2_r4(simd, data, p, p_div, inv_twid, 0, 0);
}

#[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
#[cfg(feature = "avx512")]
pub(crate) fn fwd_avx512_r4(simd: crate::V4, data: &mut [u32], p: u32, p_div: Div32, twid: &[u32]) {
    let p_div = p_div.double_reciprocal;
    let p_div = (
        p_div as u32,
        (p_div >> 32) as u32,
    ...
    fwd_depth_first_avx512_r4(simd, data, p, p_div, twid, 0, 0);
}
#[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
#[cfg(feature = "avx512")]
pub(crate) fn inv_avx512_r4(
    ...
    inv_depth_first_avx512_r4(simd, data, p, p_div, inv_twid, 0, 0);
}
...
mod tests {
    ...
    fn test_product() {
        for n in [2, 4, 8, 16, 32, 64, 128, 256, 512, 1024] {
            ...
            fwd_scalar_r4(&mut lhs_fourier, p, Div32::new(p), &twid);
            fwd_scalar_r4(&mut rhs_fourier, p, Div32::new(p), &twid);

            for i in 0..n {
                prod[i] = mul(p, lhs_fourier[i], rhs_fourier[i]);
            }

            inv_scalar_r4(&mut prod, p, Div32::new(p), &inv_twid);
            let result = prod;
    ...
    fn test_product_avx2() {
        if let Some(simd) = crate::V3::try_new() {
            for n in [32, 64, 128, 256, 512, 1024] {
                ...
                fwd_avx2_r4(simd, &mut lhs_fourier, p, Div32::new(p), &twid);
                fwd_avx2_r4(simd, &mut rhs_fourier, p, Div32::new(p), &twid);

                for i in 0..n {
                    prod[i] = mul(p, lhs_fourier[i], rhs_fourier[i]);
                }

                inv_avx2_r4(simd, &mut prod, p, Div32::new(p), &inv_twid);
                let result = prod;
    ...
    fn test_product_avx512() {
        if let Some(simd) = crate::V4::try_new() {
            for n in [32, 64, 128, 256, 512, 1024] {
                ...
                fwd_avx512_r4(simd, &mut lhs_fourier, p, Div32::new(p), &twid);
                fwd_avx512_r4(simd, &mut rhs_fourier, p, Div32::new(p), &twid);

                for i in 0..n {
                    prod[i] = mul(p, lhs_fourier[i], rhs_fourier[i]);
                }

                inv_avx512_r4(simd, &mut prod, p, Div32::new(p), &inv_twid);
                let result = prod;
