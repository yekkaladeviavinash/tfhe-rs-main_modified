...
fn main() {
    // Define NTT prime and polynomial size (must be power of 4 for radix-4)
    let p: u32 = 1073479681;
    let polynomial_size: usize = 2048; // 64 = 4^3, suitable for radix-4
    ...
    // println!("Left polynomial (first 8):  {:?}", &lhs_poly[..8]);
    // println!("Right polynomial (first 8): {:?}", &rhs_poly[..8]);
    ...
    // Method 2: Custom Radix-4 NTT from fwd_1.rs
    ...
    let psi = pow_mod(g, (p - 1) / (two_n as u32), p);  // 2n-th root of unity
    let omega = mul_mod(psi, psi, p);  // n-th root of unity
    ...
    // Forward NTT using radix-4
    fft_radix4_recursive_mut(&mut lhs_ntt, &tw, p, &mut stats);
    fft_radix4_recursive_mut(&mut rhs_ntt, &tw, p, &mut stats);
    ...
    // Inverse NTT using radix-4
    ifft_radix4_recursive_mut(&mut lhs_ntt, &inv_tw, p, n_inv, true, &mut stats);
    ...
    println!("\nRadix-4 NTT multiplication time: {:?}", duration);
    ...
    // println!("\nMultiplied result (first 8): {:?}", &lhs_ntt[..8]);
    // println!("Success! Radix-4 NTT result matches schoolbook algorithm.");
}
