...
fn main() {
    ...
    let polynomial_size: usize = 2048; // Power of 2, suitable for split-radix
    ...
    // Method 2: Custom Split-Radix NTT from fwd_1.rs
    ...
    // Forward NTT using split-radix
    fft_split_radix_recursive_mut(&mut lhs_ntt, &tw, p, &mut stats);
    fft_split_radix_recursive_mut(&mut rhs_ntt, &tw, p, &mut stats);
    ...
    // Inverse NTT using split-radix
    ifft_split_radix_recursive_mut(&mut lhs_ntt, &inv_tw, p, n_inv, true, &mut stats);
    ...
    println!("\nSplit-Radix NTT multiplication time: {:?}", duration);
    ...
    // println!("Success! Split-Radix NTT result matches schoolbook algorithm.");
}
